...
/** ************************************************************************************************************** */
const pageTitle = 'Etudes en cours';
...
$.getJSON(
  'http://10.1.2.7:3000/requests/etudeencours/0/' +
    capitalref +
    '/' +
    comref +
    '/' +
  ...
  function(data) {
    $.each(data, function(key, result) {
      if (result.avancement_etude_percent_plan_implantation === 100) {
        return;
      }
      ...
      if (
        ...
        result.avancement_etude_date_plan_implantation = formatDate(
          result.avancement_etude_date_plan_implantation
        );
      } else {
        result.avancement_etude_date_plan_implantation = 'Inconnue';
      }
      if (result.avancement_etude_percent_plan_implantation !== 100) {
        enCoursEtude++;
      }
    ...
    $('#EnCours').html(enCoursEtude);
    $('#EnRetard').html(enRetardEtude);
    // Generation du tableau
    $(document).ready(function() {
      let table = $('#table').DataTable({
        columns: [
          { data: 'retard' },
          { data: 'avancement_etude_date_plan_implantation' },
          { data: 'numero_cv' },
          { data: 'pseudo_client' },
          { data: 'Description_Offer' },
          { data: 'affectation_etude' },
          { data: 'avancement_etude_percent_plan_implantation' },
